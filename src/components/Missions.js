...
function Missions() {
  ...
  const handleClick = (id, join) => {
    dispatch(joinLeaveMission({ id:id, joinOrLeave: join }));
  };
  return (
      ...
      <table>
        ...
        {missions.map((mission) => (
          <tr key={mission.mission_id}>
            ...
            <td>
              {mission.joined
                ? (<button className="mission_leave" onClick={() => handleClick(mission.mission_id, false)}>Leave mission</button>)
                : (<button className="mission_join" onClick={() => handleClick(mission.mission_id, true)}>Join Mission</button>)}
            </td>
          </tr>
        ))}
        </tbody>
